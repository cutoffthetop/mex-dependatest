...
container:
	...
	@ echo building docker image mex-dependatest:${LATEST}; \
	export DOCKER_BUILDKIT=1; \
	docker build \
		--tag rki/mex-dependatest:${LATEST} \
		--tag rki/mex-dependatest:latest .; \

run: container
	# run the service as a docker container
	@ echo running docker container mex-dependatest:${LATEST}; \
	docker run \
		--env MEX_DEPENDATEST_HOST=0.0.0.0 \
		--publish 8081:8081 \
		rki/mex-dependatest:${LATEST}; \

start: container
	# start the service using docker-compose
	@ echo running docker-compose with mex-dependatest:${LATEST}; \
	export DOCKER_BUILDKIT=1; \
